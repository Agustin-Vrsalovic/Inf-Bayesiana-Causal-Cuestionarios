...
def p_csr_p(r: int, c: int, s: int, p: float) -> float:
    ...
    p ∈ [0, 1]
    """
    return p_rcs_M(r,c,s,m=1) * p + p_rcs_M(r,c,s,m=0) * (1-p)
...
def p_p_datos(p: float, datos: List[Tuple[int, int, int]]) -> float:
    ...
    for i in range(len(datos)):
        productoria *= p_csr_p(c=datos[i][0],
                               s=datos[i][1],
                               r=datos[i][2],
                               p=p)
...
def posterior_p(grilla, datos):
    ...
    for p in grilla:
        valores.append(p_p_datos(p, datos))

    normalizador = sum(valores)

    return [valor / normalizador for valor in valores]
...
def estimar_p(posterior_p: Callable, observaciones: int = 60) -> pd.DataFrame:
    ...
    no_monty = pd.read_csv("CSVs/NoMontyHall.csv")
...
def p_datos_MA(datos: List[Tuple[int, int, int]]) -> float:
    ...
    for episodio in range(len(datos)):
        productoria *= p_episodio_datos_MA(c=datos[episodio][0], s=datos[episodio][1],
                                           r=datos[episodio][2], datos=episodios_anteriores)
        episodios_anteriores.append(datos[episodio])
...
def log_p_datos_MA(datos: List[Tuple[int, int, int]]) -> float:
    ...
    for episodio in range(len(datos)):
        sumatoria_log += np.log10(p_episodio_datos_MA(c=datos[episodio][0], s=datos[episodio][1],
                                                      r=datos[episodio][2], datos=episodios_anteriores))
        episodios_anteriores.append(datos[episodio])

    return sumatoria_log
...
if __name__ == "__main__":
    # Carga de datos
    no_monty_df = pd.read_csv("CSVs/NoMontyHall.csv")
